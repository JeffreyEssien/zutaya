...
// iOS Safari only honours ONE programmatic download per gesture, so a loop of
// <a download> clicks drops all but one slide. Bundling the PNGs into a single
// zip means one download grabs them all. PNGs are already compressed, so the
// "store" method (no deflate) keeps this dependency-free and tiny.
...
async function zipStore(files: { name: string; blob: Blob }[]): Promise<Blob> {
    ...
    const eocd = new Uint8Array([
        ...
        ...u16(0), ...u16(0), // disk #, central dir disk
        ...u16(files.length), ...u16(files.length),
        ...u32(centralSize), ...u32(offset), // central size, central offset
        ...u16(0), // comment len
    ]);
...
export async function generateCataloguePngs(
    ...
    if (variant === "photos") {
        ...
        files.push({
            name: `${slug}-catalogue-00-cover-${stamp}.png`,
            blob: await renderCoverSlide(),
        });
        for (let i = 0; i < pages.length; i++) {
            ...
            files.push({
                name: `${slug}-catalogue-${n}-${slugify(page.category)}${partSuffix}-${stamp}.png`,
                blob: await renderPhotoImage(page, images),
            });
        }
    } else {
        ...
        for (let i = 0; i < pages.length; i++) {
            ...
            const partSuffix = page.totalParts > 1 ? `-${page.part}` : "";
            files.push({
                name: `${slug}-catalogue-prices-${slugify(page.category)}${partSuffix}-${stamp}.png`,
                blob: await renderListImage(page),
            });
        }
    }

    // One image → download it directly. Multiple → bundle into a single zip so
    // it works on iOS Safari (which drops all but one looped download).
    if (files.length === 1) {
        downloadBlob(files[0].blob, files[0].name);
    } else {
        const kind = variant === "photos" ? "photo-catalogue" : "price-list";
        downloadBlob(await zipStore(files), `${slug}-${kind}-${stamp}.zip`);
    }
    return files.length;
}
