                    ...
                    >
                        ...
                        <div className="flex items-center justify-between px-6 py-5 border-b border-warm-cream/10">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="font-serif text-lg text-warm-cream">Cart</h2>
                                <span className="bg-brand-green/10 text-brand-green text-[10px] font-bold px-2.5 py-0.5 rounded-full">
                                    {items.length}
                                </span>
                            </div>
                            <button
                                type="button"
                                onClick={close}
                                className="p-2 hover:bg-warm-cream/5 rounded-full transition-colors cursor-pointer"
                        ...
                        {items.length > 0 && (
                            <div className="px-6 py-3 bg-gradient-to-r from-brand-lilac/[0.03] to-brand-purple/[0.03] border-b border-warm-cream/8">
                                <p className="text-xs text-warm-cream/40 flex items-center gap-2">
                                    <Truck size={13} className="text-brand-green shrink-0" />
                        ...
                        <div className="flex-1 overflow-y-auto px-6 py-4">
                            {items.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full text-center py-12">
                                    <div className="p-5 bg-warm-cream/5 rounded-full mb-5">
                                        <ShoppingBag size={28} className="text-warm-cream/15" />
                                    </div>
                                    <p className="text-warm-cream/40 mb-1 font-medium">Your cart is empty</p>
                                    <p className="text-xs text-warm-cream/25 mb-6">Browse our collection and add items</p>
                        ...
                        {/* Footer */}
                        {items.length > 0 && (
                            <div className="border-t border-warm-cream/10 px-6 py-5 space-y-3 bg-warm-cream/5/30">
                                ...
                                <div className="flex justify-between text-sm text-warm-cream/50">
                                    <span>Shipping</span>
                                    <span className="text-xs text-warm-cream/30 italic">Calculated at checkout</span>
                                </div>
                                <div className="flex justify-between font-semibold text-warm-cream pt-3 border-t border-warm-cream/8">
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-between">
                <div>
                    <p className={`font-medium text-warm-cream truncate ${compact ? "text-xs" : "text-sm"}`}>{item.product.name}</p>
                    {item.variant && <p className="text-[10px] text-warm-cream/35 mt-0.5">{item.variant.name}</p>}
                    ...
                    <p className={`font-semibold text-warm-cream mt-0.5 ${compact ? "text-xs" : "text-sm"}`}>{formatCurrency(price * item.quantity)}</p>
                </div>
                <div className="flex items-center justify-between mt-1.5">
                    ...
                    {!compact && (
                        <button type="button" onClick={() => removeItem(item.product.id, item.variant?.name)} className="p-1.5 hover:bg-red-50 rounded-full transition-colors cursor-pointer group">
                            <Trash2 size={13} className="text-warm-cream/20 group-hover:text-red-500 transition-colors" />
                        </button>
                    )}
                </div>
            </div>
