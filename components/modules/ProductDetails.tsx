...
export default function ProductDetails({ product }: ProductDetailsProps) {
    ...
    return (
        ...
        >
            {/* Breadcrumb */}
            <nav className="flex items-center gap-1.5 text-xs text-warm-cream/35 mb-6">
                ...
                <ChevronRight size={10} />
                <span className="text-warm-cream/60 truncate max-w-[120px]">{product.name}</span>
            </nav>

            {/* Category + Title */}
            <p className="text-[10px] uppercase tracking-[0.25em] text-brand-green font-semibold mb-3">{product.category}</p>
            ...
            <div className="flex items-baseline gap-3 mb-6">
                <p className="font-serif text-3xl font-bold text-warm-cream">
                    {formatCurrency(currentPrice)}
                    {priceUnitLabel && <span className="text-lg font-normal text-warm-cream/40 ml-1">{priceUnitLabel}</span>}
                </p>
            </div>

            {/* Description Tabs */}
            <div className="mb-8">
                <div className="flex gap-6 border-b border-warm-cream/10 mb-4">
                ...
                <AnimatePresence mode="wait">
                    {activeTab === "description" ? (
                        <motion.div
                            ...
                            className="product-description text-warm-cream/55 leading-relaxed text-[15px]"
                        ...
                        {product.variants.map((v) => (
                            <button
                                ...
                                className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 cursor-pointer ${selectedVariant?.name === v.name
                                    ...
                                    : "bg-warm-cream/5 text-warm-cream/70 hover:bg-warm-cream/10 border border-warm-cream/8 hover:border-warm-cream/20"
                                    }`}
                            >
                                {v.name}
                            </button>
...
function TrustBadge({ icon: Icon, label, sublabel }: { icon: React.ElementType; label: string; sublabel: string }) {
    return (
        <div className="text-center p-3 rounded-xl bg-warm-cream/5/70 border border-warm-cream/8 hover:border-warm-cream/20 transition-colors group">
            <Icon size={16} className="mx-auto text-brand-green/70 mb-1.5 group-hover:text-brand-green transition-colors" />
            <p className="text-[10px] font-semibold text-warm-cream/70">{label}</p>
            <p className="text-[9px] text-warm-cream/30 mt-0.5">{sublabel}</p>
        </div>
    );
}
...
function DetailRow({ label, value }: { label: string; value: string }) {
    return (
        <div className="flex justify-between text-sm py-2 border-b border-warm-cream/5 last:border-0">
            <span className="text-warm-cream/35 text-xs uppercase tracking-wide">{label}</span>
            <span className="text-warm-cream font-medium capitalize text-sm">{value}</span>
        </div>
    );
}
