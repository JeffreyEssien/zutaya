...
const statusConfig = {
    success: { icon: CheckCircle, color: "text-emerald-400", bg: "bg-emerald-500/10", variant: "success" as const },
    error: { icon: XCircle, color: "text-red-400", bg: "bg-red-500/10", variant: "danger" as const },
    skipped: { icon: SkipForward, color: "text-amber-400", bg: "bg-amber-500/10", variant: "warning" as const },
};
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                <div>
                    ...
                    <p className="text-sm text-warm-cream/50 mt-1">Automated tasks powered by Vercel Cron</p>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <StatCard label="Successful" value={successCount} accent="text-emerald-400" />
                <StatCard label="Errors" value={errorCount} accent="text-red-500" />
                <StatCard label="Items Processed" value={totalProcessed} accent="text-brand-green" />
            </div>
                        ...
                        <div key={job.key} className="bg-white/[0.04] rounded-xl border border-warm-cream/15 p-5 shadow-sm space-y-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="font-semibold text-warm-cream text-sm">{job.label}</h3>
                                    <p className="text-xs text-warm-cream/40 mt-0.5">{job.desc}</p>
                                </div>
                            </div>

                            <div className="flex items-center gap-2 text-xs text-warm-cream/40">
                                <Clock size={11} />
                            ...
                            {lastRun && (
                                <div className={`flex items-center gap-2 px-3 py-2 rounded-lg ${lastStatus?.bg}`}>
                                    ...
                                    <span className="text-[10px] text-warm-cream/30 ml-auto">
                            ...
                            {lastRun?.details && (
                                <p className="text-[11px] text-warm-cream/40 leading-relaxed line-clamp-2">{lastRun.details}</p>
                            )}
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-warm-cream/10 bg-warm-cream/[0.03]">
                                        <th className="text-left px-4 py-3 text-[10px] font-semibold text-warm-cream/40 uppercase tracking-wider">Job</th>
                                        <th className="text-left px-4 py-3 text-[10px] font-semibold text-warm-cream/40 uppercase tracking-wider">Status</th>
                                        <th className="text-left px-4 py-3 text-[10px] font-semibold text-warm-cream/40 uppercase tracking-wider">Details</th>
                                        <th className="text-center px-4 py-3 text-[10px] font-semibold text-warm-cream/40 uppercase tracking-wider">Processed</th>
                                        <th className="text-right px-4 py-3 text-[10px] font-semibold text-warm-cream/40 uppercase tracking-wider">Time</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-warm-cream/5">
                                    {logs.map((log) => {
                                        ...
                                        return (
                                            <tr key={log.id} className="hover:bg-warm-cream/[0.03]">
                                                <td className="px-4 py-3 font-medium text-warm-cream">{job?.label || log.job_name}</td>
                                                <td className="px-4 py-3"><Badge variant={cfg.variant}>{log.status}</Badge></td>
                                                <td className="px-4 py-3 text-warm-cream/50 text-xs max-w-xs truncate">{log.details || "—"}</td>
                                                <td className="px-4 py-3 text-center text-warm-cream/60 font-mono">{log.items_processed}</td>
                                                <td className="px-4 py-3 text-right text-warm-cream/40 text-xs">
                            ...
                            {logs.map((log) => {
                                ...
                                return (
                                    <div key={log.id} className="bg-white/[0.04] rounded-lg border border-warm-cream/15 p-4 space-y-2">
                                        ...
                                        {log.details && <p className="text-xs text-warm-cream/40 line-clamp-2">{log.details}</p>}
                                        <div className="flex items-center justify-between text-[10px] text-warm-cream/30">
