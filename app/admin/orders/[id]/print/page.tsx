    ...
    return (
        <div className="max-w-3xl mx-auto p-8 bg-white/[0.04] print:p-0">
            ...
            <div className="flex justify-between items-start border-b border-black/10 pb-8 mb-8">
                <div>
                    <h1 className="font-serif text-3xl text-warm-cream mb-2">{SITE_NAME}</h1>
                    <p className="text-sm text-warm-cream/60">Thank you for your business.</p>
                </div>
                <div className="text-right">
                    <h2 className="text-xl font-medium text-warm-cream uppercase tracking-wide">Packing Slip</h2>
                    <p className="text-sm text-warm-cream/60 mt-1">Order #{order.id}</p>
                    <p className="text-sm text-warm-cream/60">{new Date(order.createdAt).toLocaleDateString()}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-12 mb-12">
                <div>
                    <h3 className="text-xs font-semibold text-warm-cream/40 uppercase tracking-wider mb-3">Ship To</h3>
                    <div className="text-sm text-warm-cream space-y-1">
                        <p className="font-medium">{order.shippingAddress.firstName} {order.shippingAddress.lastName}</p>
                        <p>{order.shippingAddress.address}</p>
                        <p>{order.shippingAddress.city}, {order.shippingAddress.state} {order.shippingAddress.zip}</p>
                        <p>{order.shippingAddress.country}</p>
                        <p className="text-warm-cream/60 mt-2">{order.email}</p>
                        <p className="text-warm-cream/60">{order.phone}</p>
                    </div>
                </div>
                <div>
                    <h3 className="text-xs font-semibold text-warm-cream/40 uppercase tracking-wider mb-3">Order Details</h3>
                    <div className="text-sm text-warm-cream space-y-1">
                    ...
                    {order.items.map((item) => (
                        <tr key={`${item.product.id}-${item.variant?.name ?? ""}`}>
                            ...
                            <td className="py-4 text-warm-cream align-top">
                                ...
                                {item.variant && (
                                    <p className="text-warm-cream/60 text-xs mt-1">Variant: {item.variant.name}</p>
                                )}
                            </td>
                            <td className="py-4 text-right text-warm-cream align-top">
            ...
            <div className="border-t border-black/10 pt-4 mb-12">
                <div className="flex justify-between text-sm mb-1">
                    <span className="text-warm-cream/60">Subtotal</span>
                ...
                <div className="flex justify-between text-sm mb-1">
                    <span className="text-warm-cream/60">Delivery</span>
            ...
            {/* Footer */}
            <div className="border-t-2 border-brand-green pt-8 text-center">
                <p className="text-warm-cream font-serif text-lg mb-2">Thank you!</p>
